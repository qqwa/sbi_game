...
const glm::mat4 Transform::GetMatrix() const {
  if (m_owner->GetParent() != nullptr &&
      m_owner->GetParent()->HasComponent<Transform>()) {
    // TODO: optimization -> cache matrix calculation per frame base
    return m_owner->GetParent()->GetComp<Transform>()->GetMatrix() *
           Matrix(m_position, m_orientation, m_scale);
  } else {
...
glm::quat Transform::GetTotalOrientation() {
  if (m_owner->GetParent() != nullptr &&
      m_owner->GetParent()->HasComponent<Transform>()) {
    // TODO: optimization -> cache matrix calculation per frame base
    return m_owner->GetParent()->GetComp<Transform>()->GetTotalOrientation() *
           m_orientation;
  } else {
